...
float4 PixelColor(in uint2 pixelCoord)
{
    ...
    float2 uv = (pixelCoord + 0.5) * _MainTex_TexelSize.xy;
...
float3 DecodeRGB565(in uint value)
{
    return float3(
        ...
        float((value >> 5u) & 63u) * (1.0 / 64.0),
        float((value >> 0u) & 31u) * (1.0 / 31.0)
    );
}

float Distance(float3 color1, float3 color2)
{
...
// return: 0-3
uint MinDistanceIndex(float4 distances)
{
    uint4 b = uint4(
    ...
    uint b4 = distances.z > distances.w ? 1u : 0u;
...
// subpixelCoord: 0-3
float3 DecodeDXT1(uint4 block, uint2 subpixelCoord)
{
...
uint2 BlockCoord(float2 uv)
{
    return PixelCoord(uv) & uint2(~3u, ~3u);
}

uint2 SubpixelCoord(uint2 pixelCoord)
{
    return pixelCoord & uint2(3u, 3u);
}

uint2 SubpixelCoord(float2 uv)
{
    return SubpixelCoord(PixelCoord(uv));
}

float4 BlockToNormalized(uint4 block)
{
    return float4(block) / float4(65536, 65536, 65536, 65536);
}

uint4 NormalizedToBlock(float4 normalized)
{
    return uint4((normalized) * float4(65536, 65536, 65536, 65536));
}
